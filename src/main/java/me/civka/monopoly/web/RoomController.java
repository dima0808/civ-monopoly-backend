...
public class RoomController {
  ...
  @Operation(summary = "Get room by reference", description = "Returns details of a room by its reference.")
  @ApiResponses(value = {
  ...
  public RoomDto getRoomByReference(
      @Parameter(description = "Room reference UUID", required = true)
      @PathVariable UUID roomReference) {
    return roomService.getRoomByReference(roomReference);
  }

  @Operation(summary = "Create a new room", description = "Creates a new room with the specified parameters.")
  @ApiResponses(value = {
  ...
  public RoomDto createRoom(
      @Parameter(description = "Room create request", required = true)
  ...
  @ApiResponses(value = {
      @ApiResponse(responseCode = "200", description = "Successfully joined the room",
          content = @Content(schema = @Schema(implementation = RoomDto.class))),
      @ApiResponse(responseCode = "400", description = "Invalid password or room is full",
          content = @Content(schema = @Schema(implementation = ProblemDetail.class))),
      @ApiResponse(responseCode = "404", description = "Room not found",
          content = @Content(schema = @Schema(implementation = ProblemDetail.class)))
  })
  ...
  public RoomDto joinRoom(
      @Parameter(description = "Room reference UUID", required = true)
      @PathVariable UUID roomReference,
      @Parameter(description = "Room join request", required = true)
  ...
  @Operation(summary = "Leave current room", description = "Leaves the room the user is currently in.")
  @ApiResponses(value = {
  ...
  @Operation(summary = "Kick user from room", description = "Kicks a user from the current room (owner only).")
  @ApiResponses(value = {
      @ApiResponse(responseCode = "200", description = "User kicked from room",
          content = @Content(schema = @Schema(implementation = RoomDto.class))),
  ...
  public RoomDto kickUser(
      @Parameter(description = "User reference UUID", required = true)
